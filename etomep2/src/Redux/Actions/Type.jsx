export const ADMIN_INFO = 'ADMIN_INFO'
export const TEACHER_INFO = 'TEACHER_INFO'
export const ADMIN_CLASS_INFO = 'ADMIN_CLASS_INFO'
export const ADMIN_TEACHER_INFO = 'ADMIN_TEACHER_INFO'
export const EXAM_PAPER_INFO = 'EXAM_PAPER_INFO'
export const TEACHER_SUBJECT_INFO = 'TEACHER_SUBJECT_INFO'
export const ADMIN_ALL_CLASS_INFO = 'ADMIN_ALL_CLASS_INFO'
export const TEACHER_LOGOUT = 'TEACHER_LOGOUT'
export const TEACHER_UPDATE = 'TEACHER_UPDATE'
export const ADMIN_SELECTED_TEXTBOOK = "ADMIN_SELECTED_TEXTBOOK";
export const ADMIN_SELECTED_LIBRARY = 'ADMIN_SELECTED_LIBRARY';
export const LOAD_EXAM_CLASSES = 'ExamClasses_LOAD'
export const LOAD_TIMETABLEDATA_QUESTIONPAPER = 'LOAD_TIMETABLEDATA_QUESTIONPAPER';

