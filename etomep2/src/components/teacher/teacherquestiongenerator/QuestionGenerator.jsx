...
function QuestionGenerator() {
      ...
      subsections.map(async (subsection, sectionIndex) => {
        const questionsWithImages = await Promise.all(
          subsection.questions.map(async (q, questionIndex) => {
            const questionElement = questionRefs.current[sectionIndex]?.[questionIndex]?.getEditorRef();
            const answerElement = answerRefs.current[sectionIndex]?.[questionIndex]?.getEditorRef();

            console.log(questionElement, "question Element "); // Debug log
            console.log(answerElement, "answer Element "); // Debug log

            const questionImage = questionElement ? await captureElement(questionElement) : "Image not captured";
            const answerImage = answerElement ? await captureElement(answerElement) : "Image not captured";
  ...
  const sendToBackend = async (sectionsWithImages) => {
    ...
    try {
      const response = await axios.post(`${APIURL}/api/questionpapersetting`, data);
          ...
          <div className="text-editor">
            {subsections.map((subsection, subsectionIndex) => (
              <Droppable key={subsectionIndex} droppableId={`droppable-${subsectionIndex}`} type="QUESTION">
                {(provided) => (
                  <div ref={provided.innerRef} {...provided.droppableProps} className="subsection-container">
                    ...
                    {subsection.questions.map((q, questionIndex) => {
                      ...
                      return (
                        <Draggable key={questionIndex} draggableId={`draggable-${subsectionIndex}-${questionIndex}`} index={questionIndex}>
                          {(provided) => (
                            <div ref={provided.innerRef} {...provided.draggableProps} className="question-container">
                              <div className="teacher_question_header">
                                ...
                                <TeacherTextEditor
                                  ref={(el) => (questionRefs.current[subsectionIndex][questionIndex] = el)}
                                  placeholder="Type question here..."
                                  editorData={q.question}
                                  setEditorData={(data) =>
                                    handleEditorData(subsectionIndex, questionIndex, "question", data)
                                  }
                                />
                              </div>
                              ...
                              <div className="teacher_question_footer">
                                <div
                                  ...
                                  onClick={() => toggleAnswerKey(subsectionIndex, questionIndex)}
                                >
                                  Answer Key
                                </div>
                                <div></div>
                              ...
                              {q.showAnswer && (
                                <div className="answer_editor_container">
                                  <TeacherTextEditor
                                    ref={(el) => (answerRefs.current[subsectionIndex][questionIndex] = el)}
                                    placeholder="Type answer here..."
                                    editorData={q.answer}
                                    setEditorData={(data) =>
                                      handleEditorData(subsectionIndex, questionIndex, "answer", data)
                                    }
                                  />
                              ...
                              <div className="question_actions">
                                <div {...provided.dragHandleProps} className="drag_handle">
                                  <PiDotsSix className="teacher_icon" />
                                </div>
                                <button
                                  className="delete_question_button"
                                  onClick={() => removeQuestion(subsectionIndex, questionIndex)}
                                >
